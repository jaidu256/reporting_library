...
(
		select
			...
			,sum(bud.revenue_amt)*DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), GETDATE())/DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), DATEADD(d, -1, DATEADD(m, DATEDIFF(m, 0, GETDATE()) + 1, 0))) as 'Revenue'
			,sum(bud.spread_amt)*DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), GETDATE())/DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), DATEADD(d, -1, DATEADD(m, DATEDIFF(m, 0, GETDATE()) + 1, 0))) as 'Spread'
			,sum(bud.volume)*DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), GETDATE())/DATEDIFF(day, DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0), DATEADD(d, -1, DATEADD(m, DATEDIFF(m, 0, GETDATE()) + 1, 0))) as 'Volume'
		...
		where
			datepart(YEAR, bud.budget_month) = 2019
		and DATEPART(MONTH, bud.budget_month) <= (datepart(MONTH, getdate()))
		group by
			cust.bridge_name
...
order by
	[Deviation $ (Spr Budget)] asc
	,[Deviation % (Spr Budget)] asc
	,[Deviation $ (Rev Budget)] asc
	,[Deviation % (Rev Budget)] asc
	,[Customer Name]