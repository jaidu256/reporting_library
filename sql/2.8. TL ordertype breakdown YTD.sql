...
select
	tot.order_type as 'Order Type'
	,sum(iif(tot.Date_Year = year(@dt), tot.Revenue, 0)) as 'Revenue MTD'
	,sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0)) as 'Revenue PY MTD'
	,iif(sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0)) = 0, null , (sum(iif(tot.Date_Year = year(@dt), tot.Revenue, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0))) / sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0))) as 'YoY Change % (Rev MTD)'
	
	,sum(iif(tot.Date_Year = year(@dt), tot.Spread, 0)) as 'Spread MTD'
	,sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0)) as 'Spread PY MTD'
	,iif(sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0)) = 0, null , (sum(iif(tot.Date_Year = year(@dt), tot.Spread, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0))) / sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0))) as 'YoY Change % (Spr MTD)'
	
	,sum(iif(tot.Date_Year = year(@dt), tot.Volume, 0)) as 'Volume MTD'
	,sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0)) as 'Volume PY MTD'
	,iif(sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0)) = 0, null , cast((sum(iif(tot.Date_Year = year(@dt), tot.Volume, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0))) as float) / sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0))) as 'YoY Change % (Vol MTD)'
from
(
		-- Current year powerbroker
		select
			'pb' as 'Source'
			,m.order_type_id as order_type
