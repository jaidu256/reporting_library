...
select
	iif(tot.Revenue_Type in ('TL', 'LTL', 'IM'), tot.revenue_type, 'All Others') as 'Revenue Type'
	...
	,(sum(iif(tot.Date_Year = year(@dt), tot.Revenue, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0))) / (sum(iif(tot.Date_Year = year(@dt_prev), tot.Revenue, 0)) + 1) as 'YoY Change % (Rev MTD)'

	,sum(iif(tot.Date_Year = year(@dt), tot.Spread, 0)) as 'Spread MTD'
	,sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0)) as 'Spread PY MTD'
	,(sum(iif(tot.Date_Year = year(@dt), tot.Spread, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0))) / (sum(iif(tot.Date_Year = year(@dt_prev), tot.Spread, 0)) + 1) as 'YoY Change % (Spr MTD)'

	,sum(iif(tot.Date_Year = year(@dt), tot.Volume, 0)) as 'Volume MTD'
	,sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0)) as 'Volume PY MTD'
	,cast((sum(iif(tot.Date_Year = year(@dt), tot.Volume, 0)) - sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0))) as float) / (sum(iif(tot.Date_Year = year(@dt_prev), tot.Volume, 0)) + 1) as 'YoY Change % (Vol MTD)'
from
(
...
group by
	iif(tot.Revenue_Type in ('TL', 'LTL', 'IM'), tot.revenue_type, 'All Others')
order by
	[Revenue Type] desc